...
func main() {
	addr := flag.String("addr", "localhost:50051", "Admin server address")
	token := flag.String("token", os.Getenv("NITELLA_TOKEN"), "Authentication token (env: NITELLA_TOKEN)")
	flag.Parse()
	...
	fmt.Printf("Nitella CLI - Connected to %s\n", serverAddr)
...
// newCompletion creates tab completion for nitella commands.
