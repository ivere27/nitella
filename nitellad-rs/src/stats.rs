...
impl StatsService {
    ...
    pub fn register_connection(
        ...
        let stats = self.proxy_stats.entry(proxy_id.clone()).or_default().clone();
        ...
        let _ = self.event_tx.send(Event {
            r#type: Some(event::Type::Connection(crate::proto::proxy::ConnectionEvent {
                conn_id: id,
                // proxy_id removed
                source_ip,
    ...
    pub fn record_approval_request(&self, ip: &str, rule: &str, proxy_id: &str, req_id: &str) {
        ...
        info!("[Local] Alert generated (pending approval): {} - proxy={} ip={} rule={}", req_id, proxy_id, ip, rule);
    ...
    pub fn get_summary(&self, proxy_id: Option<&str>) -> (i64, i64, i64, i64) { // active, total, in, out
        if let Some(pid) = proxy_id {
            let active = self.active_conns.iter().filter(|e| e.proxy_id == pid).count() as i64;
