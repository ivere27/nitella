...
use crate::proto::proxy::{HealthStatus, HealthCheckType};
...
impl HealthChecker {
    ...
    async fn check_all(&self) {
        ...
        for (id, p) in proxies.iter() {
            ...
            if let Some(hc) = &p.config.health_check {
                ...
                let interval_secs = if interval_ms > 0 { interval_ms / 1000 } else { 5 };
                ...
                let check_type = HealthCheckType::try_from(hc.r#type).unwrap_or(HealthCheckType::Unspecified);
                ...
                let timeout_ms = if parsed_timeout > 0 { parsed_timeout } else { 2000 };
                let id_clone = id.clone();

                tokio::spawn(async move {
                    let result = match check_type {
                        HealthCheckType::Tcp => Self::check_tcp(&target, timeout_ms).await,
                        HealthCheckType::Http | HealthCheckType::Https => Self::check_http(&target, &path, expected_status, timeout_ms).await,
                        _ => true, 
                    };
                    ...
                    if old != new_status {
                        debug!("Health status changed for {}: {} -> {}", id_clone, old, new_status);
                    }
                });
            }
        }
    ...
    async fn check_tcp(target: &str, timeout_ms: u64) -> bool {
        match tokio::time::timeout(Duration::from_millis(timeout_ms), TcpStream::connect(target)).await {
            Ok(Ok(_)) => true,
            _ => false,
        }
    }
