...
impl L2Cache {
    ...
    pub async fn put(&self, ip: &str, info: &GeoInfo) {
        ...
        .bind(ip)
        .bind(&info.country).bind(&info.country_code).bind(&info.region).bind(&info.region_name)
        .bind(&info.city).bind(&info.zip).bind(info.latitude).bind(info.longitude)
        .bind(&info.timezone).bind(&info.isp).bind(&info.org).bind(&info.r#as).bind(&info.source)
        .bind(now)
        .execute(&self.pool)
        .await;
    }
...
impl GeoIPService {
    pub async fn new(
        ...
        let strategy = if let Some(s) = strategy_str {
            ...
            vec!["l1".to_string(), "l2".to_string(), "local".to_string(), "remote".to_string()]
        };
    ...
    pub async fn lookup(&self, ip_str: &str) -> GeoInfo {
        ...
        for step in &state.strategy {
            match step.as_str() {
                ...
                "remote" => {
                    if let Some(info) = self.lookup_remote(&state, ip_str).await {
                        self.cache_result(&state, ip_str, &info).await;
                        return info;
                    }
                },
    ...
    fn lookup_local(&self, state: &GeoIPState, ip_str: &str) -> Option<GeoInfo> {
             ...
             match reader.lookup::<maxminddb::geoip2::City>(ip) {
                Ok(city) => {
                    ...
                    if let Some(c) = city.country {
                        if let Some(iso) = c.iso_code { info.country = iso.to_string(); info.country_code = iso.to_string(); }
                    }
                    if let Some(subs) = city.subdivisions {
                        if let Some(sub) = subs.get(0) {
                            if let Some(iso) = sub.iso_code { info.region = iso.to_string(); }
                            if let Some(names) = &sub.names {
                                if let Some(n) = names.get("en") { info.region_name = n.to_string(); }
                            }
                        }
                    }
                    if let Some(c) = city.city {
                         if let Some(names) = &c.names {
                            if let Some(n) = names.get("en") { info.city = n.to_string(); }
                        }
                    }
                    if let Some(l) = city.location {
                        if let Some(lat) = l.latitude { info.latitude = lat; }
                        if let Some(lon) = l.longitude { info.longitude = lon; }
                        if let Some(tz) = l.time_zone { info.timezone = tz.to_string(); }
                    }
                    found = true;
                },
                Err(_) => {
        ...
        if let Some(reader) = &state.isp_reader {
             if let Ok(isp) = reader.lookup::<maxminddb::geoip2::Isp>(ip) {
                 if let Some(n) = isp.isp { info.isp = n.to_string(); }
                 if let Some(o) = isp.organization { info.org = o.to_string(); }
                 if let Some(a) = isp.autonomous_system_organization { info.r#as = a.to_string(); }
                 found = true;
             }
        }
    ...
    async fn lookup_remote(&self, state: &GeoIPState, ip_str: &str) -> Option<GeoInfo> {
        for url_fmt in &state.remote_providers {
            let url = url_fmt.replace("%s", ip_str);
            debug!("GeoIP Remote lookup: {}", url);
            match tokio::time::timeout(std::time::Duration::from_millis(3000), self.http_client.get(&url).send()).await {
                Ok(Ok(resp)) => {
                    if let Ok(json) = resp.json::<serde_json::Value>().await {
                         let mut info = GeoInfo::default();
                         info.source = "remote".to_string();
                         if let Some(s) = json.get("countryCode").and_then(|v| v.as_str()) { info.country_code = s.to_string(); }
                         if let Some(s) = json.get("country").and_then(|v| v.as_str()) { info.country = s.to_string(); }
                         if let Some(s) = json.get("city").and_then(|v| v.as_str()) { info.city = s.to_string(); }
                         if let Some(s) = json.get("isp").and_then(|v| v.as_str()) { info.isp = s.to_string(); }
                         return Some(info);
                    }
                },
                _ => {}
    ...
    pub async fn set_remote_url(&self, url: String) {
        ...
        state.remote_providers = vec![format!("{}/%s", url)];
    }
    ...
    pub async fn reload_local_db(&self, city_path: Option<String>, isp_path: Option<String>) -> Result<()> {
    ...
    pub async fn get_status(&self) -> String {
        ...
        status.push(format!("City DB: {}", if state.city_reader.is_some() { "Loaded" } else { "Not Loaded" }));
        status.push(format!("ISP DB: {}", if state.isp_reader.is_some() { "Loaded" } else { "Not Loaded" }));
        status.push(format!("L2 Cache: {}", if state.l2_cache.is_some() { "Enabled" } else { "Disabled" }));
        status.push(format!("Remote Providers: {}", state.remote_providers.len()));
        status.join("\n")
    }
