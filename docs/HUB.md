...
```
┌─────────────────────────────────────────────────────────────────────┐
│                     CLI (Local Storage Only)                        │
│                                                                     │
│  ┌─────────────────────────────────────────────────────────────┐   │
│  │  userSecret = random(32 bytes)                              │   │
│  │  (Generated once, stored locally, NEVER sent to Hub)        │   │
│  └─────────────────────────────────────────────────────────────┘   │
│                              │                                      │
│                              ▼                                      │
│  ┌─────────────────────────────────────────────────────────────┐   │
│  │  For each paired node:                                      │   │
│  │                                                             │   │
│  │    routingToken = HMAC-SHA256(nodeID, userSecret)           │   │
│  │                                                             │   │
│  │    Example:                                                 │   │
│  │      nodeID = "my-laptop"                                   │   │
│  │      userSecret = 0x1a2b3c...                               │   │
│  │      routingToken = "a7f3b2c1d4e5..."  (64 hex chars)       │   │
│  └─────────────────────────────────────────────────────────────┘   │
│                                                                     │
│  Stored: ~/.nitella/user_secret (encrypted)                        │
│          ~/.nitella/routing_tokens.json                            │
└─────────────────────────────────────────────────────────────────────┘
```
...
│  Nodes Table:                                                       │
│  ┌────────────────┬─────────────────────┬──────────────────────┐   │
│  │ routing_token  │ encrypted_metadata  │ status               │   │
│  ├────────────────┼─────────────────────┼──────────────────────┤   │
│  │ a7f3b2c1d4...  │ <encrypted blob>    │ online               │   │
│  │ x9y8z7w6v5...  │ <encrypted blob>    │ offline              │   │
│  │ m1n2o3p4q5...  │ <encrypted blob>    │ online               │   │
│  └────────────────┴─────────────────────┴──────────────────────┘   │
│                                                                     │
│  Hub CAN see:     ✓ routing_token (opaque string)                  │
│                   ✓ node status (online/offline)                   │
│                   ✓ connection timestamps                          │
│                                                                     │
│  Hub CANNOT see:  ✗ userSecret (never leaves CLI)                  │
│                   ✗ which user owns which node                     │
│                   ✗ node names, IPs, or metadata (E2E encrypted)   │
│                   ✗ command contents (E2E encrypted)               │
│                   ✗ metrics data (E2E encrypted)                   │
│                                                                     │
└─────────────────────────────────────────────────────────────────────┘
```
...
┌─────────────────────────────────────────────────────────────────────┐
│                                                                     │
│  Alice (userSecret_A)              Bob (userSecret_B)               │
│  ┌─────────────────────┐          ┌─────────────────────┐          │
│  │ nodeID: "laptop"    │          │ nodeID: "laptop"    │          │
│  │                     │          │                     │          │
│  │ HMAC(laptop, sec_A) │          │ HMAC(laptop, sec_B) │          │
│  │         │           │          │         │           │          │
│  │         ▼           │          │         ▼           │          │
│  │ token: "a7f3b2..."  │          │ token: "x9y8z7..."  │          │
│  └─────────────────────┘          └─────────────────────┘          │
│           │                                │                        │
│           └────────────┐    ┌──────────────┘                        │
│                        ▼    ▼                                       │
...
│                   ├─────────────┤                                   │
│                   │ Sees only:  │                                   │
│                   │ • a7f3b2... │  ← Cannot tell this is Alice's    │
│                   │ • x9y8z7... │  ← Cannot tell this is Bob's      │
│                   │             │                                   │
│                   │ Cannot:     │                                   │
│                   │ • Link tokens to users                          │
│                   │ • Know both are named "laptop"                  │
│                   │ • Correlate ownership                           │
│                   └─────────────┘                                   │
│                                                                     │
└─────────────────────────────────────────────────────────────────────┘
...
└──────┬──────┘                                       └──────┬──────┘
       │                                                     │
       │  1. Generate pairing code: "APPLE-BEACH-CORAL"      │
       │◄────────────────────────────────────────────────────│
       │     (User reads code aloud or types it)             │
       │                                                     │
       │  2. Both derive PAKE session from code              │
       │     cliSession = PAKE(code, role=CLI)               │
       │     nodeSession = PAKE(code, role=Node)             │
       │                                                     │
       │  3. Exchange PAKE init messages                     │
       │─────────────────────────────────────────────────────│
       │◄────────────────────────────────────────────────────│
       │                                                     │
       │  4. Derive shared key + confirmation emoji          │
       │     CLI shows: "🍎🏖️🪸"                              │
       │     Node shows: "🍎🏖️🪸"                             │
       │     (User verifies they match)                      │
       │                                                     │
       │  5. Node sends encrypted CSR                        │
       │◄────────────────────────[ E2E Encrypted ]───────────│
       │                                                     │
       │  6. CLI signs CSR with Root CA                      │
       │     CLI generates: routingToken = HMAC(nodeID, userSecret)
       │                                                     │
       │  7. CLI sends encrypted cert + routing token        │
       │─────────────────────────[ E2E Encrypted ]──────────►│
       │                                                     │
       │  8. Node stores cert, connects to Hub               │
       │     CLI stores routingToken locally                 │
       │                                                     │
       ▼                                                     ▼
  ┌─────────────────────────────────────────────────────────────┐
  │                           HUB                               │
  │  Receives: routingToken (during ApproveNode)                │
  │  Stores: routing_token ↔ node mapping                       │
  │  Does NOT know: which user, what the node name is           │
  └─────────────────────────────────────────────────────────────┘
```
...
├─────────────────────────────────────────────────────────────────────┤
│                                                                     │
│  1. Generate node keypair + CSR                                     │
│  2. Generate CPACE words for authentication                         │
│  3. Generate temporary TLS cert for HTTPS                           │
│  4. Start HTTPS server on :8888                                     │
│  5. Print to stdout (docker logs):                                  │
│                                                                     │
│  ╔══════════════════════════════════════════════════════════════╗  │
│  ║  CPACE Words:  7-tiger-castle                                ║  │
│  ║  Fingerprint:  🐶🌸🎸⭐                                       ║  │
...
└─────────────────────────────────────────────────────────────────────┘
                                ...
                                ▼
┌─────────────────────────────────────────────────────────────────────┐
│  User opens https://localhost:8888 in browser                       │
├─────────────────────────────────────────────────────────────────────┤
│                                                                     │
│  Step 1: Enter CPACE words (from docker logs)                       │
│  ┌─────────────────────────────────────────────────────────────┐   │
│  │   Enter CPACE words: [7-tiger-castle]  [Verify]             │   │
│  └─────────────────────────────────────────────────────────────┘   │
│                                                                     │
│  Step 2: Scan QR or paste signed certificate                        │
│  ┌─────────────────────────────────────────────────────────────┐   │
│  │   [QR CODE]  Fingerprint: 🐶🌸🎸⭐                          │   │
│  │                                                             │   │
│  │   Paste certificate: [________________]  [Submit]           │   │
│  └─────────────────────────────────────────────────────────────┘   │
│                                                                     │
│  Step 3: Verify CA fingerprint                                      │
│  ┌─────────────────────────────────────────────────────────────┐   │
│  │   ⚠️ VERIFY THIS MATCHES YOUR CLI:                          │   │
│  │   CA Fingerprint: 🦊🌺🎹🌙                                  │   │
│  │                                                             │   │
│  │   [Cancel]  [Confirm]                                       │   │
│  └─────────────────────────────────────────────────────────────┘   │
...
├─────────────────────────────────────────────────────────────────────┤
│                                                                     │
│  • Pairing port closed (8888)                                       │
│  • Connect to Hub with mTLS                                         │
│  • NO listening ports (until proxy configured)                      │
│  • Wait for commands from CLI via Hub                               │
│                                                                     │
│  stdout:                                                            │
│  ┌─────────────────────────────────────────────────────────────┐   │
│  │  [Pairing] Complete! CA fingerprint: 🦊🌺🎹🌙               │   │
│  │  [Hub] Connected to hub.example.com:50052                   │   │
│  │  [Hub] Waiting for commands...                              │   │
│  └─────────────────────────────────────────────────────────────┘   │
│                                                                     │
└─────────────────────────────────────────────────────────────────────┘
```
...
└──────┬──────┘                              └──────┬──────┘
       │                                            │
       │         1. nitellad outputs CSR           │
       │            (QR on screen + JSON)          │
       │◄───────── User looks at screen ────────────│
       │                                            │
       │  2. CLI scans QR (phone camera)            │
       │     or user copies JSON text               │
       │                                            │
       │  3. CLI signs CSR, outputs cert            │
       │     (as JSON)                              │
       │                                            │
       │         4. User PASTES JSON text           │
       │────────── into nitellad terminal ─────────►│
       │                                            │
       │         5. nitellad reads via stdin        │
       │            parses JSON, saves cert         │
       │            connects to Hub                 │
       │                                            │
```
...
└─────────────────────────────────────────────────────────────────────┘
  ...
  │                                                                   │
  │  2. ECDH with node's public key                                   │
  │     shared = X25519(ephemeral_priv, node_pub)                     │
  │                                                                   │
  │  3. Derive encryption key                                         │
  │     key = HKDF-SHA256(shared, "nitella-cmd")                      │
  │                                                                   │
  │  4. Encrypt command                                               │
  ...
  └─────────────┘
         ...
         ▼
  ┌─────────────┐
  │    HUB      │  Sees only: opaque encrypted blob
  │             │  Cannot: decrypt, read command, know contents
  └─────────────┘
         │
         ▼
  ┌─────────────┐
  │   Node      │
  │             │
  │  1. Extract ephemeral_pub from message                            │
  │                                                                   │
  │  2. ECDH with node's private key                                  │
  │     shared = X25519(node_priv, ephemeral_pub)                     │
  │                                                                   │
  ...
  │                                                                   │
  │  5. Execute command, encrypt response the same way                │
  └─────────────┘
```
...
│                                                                             │
│  ┌─────────────────────────┐         ┌─────────────────────────┐           │
│  │      Mobile App         │         │      nitella CLI        │           │
│  │  (Root CA holder)       │         │                         │           │
│  │                         │         │                         │           │
│  │  • Root CA private key  │         │  • userSecret           │           │
│  │  • Signs node certs     │         │  • routingTokens        │           │
│  │  • E2E encryption keys  │         │  • E2E encryption keys  │           │
│  └───────────┬─────────────┘         └───────────┬─────────────┘           │
│              │                                   │                          │
│              │ JWT + mTLS                        │ JWT + mTLS               │
│              │                                   │                          │
...
│                                                                             │
│  ┌─────────────────┐  ┌─────────────────┐  ┌─────────────────┐             │
│  │  AuthService    │  │  MobileService  │  │  NodeService    │             │
│  │                 │  │                 │  │                 │             │
│  │  • Register     │  │  • ListNodes    │  │  • Register     │             │
│  │  • Login        │  │  • SendCommand  │  │  • WatchReg     │             │
│  │  • RefreshToken │  │  • StreamMetric │  │  • ReceiveCmd   │             │
│  └─────────────────┘  └─────────────────┘  └─────────────────┘             │
│                                                                             │
│  ┌─────────────────────────────────────────────────────────────────────┐   │
│  │                         DATABASE                                     │   │
│  │                                                                      │   │
│  │  routing_tokens ──► nodes (encrypted_metadata, status)              │   │
│  │                                                                      │   │
│  │  NO user ↔ node correlation!                                        │   │
│  └─────────────────────────────────────────────────────────────────────┘   │
│                                                                             │
└─────────────────────────────────────────────────────────────────────────────┘
               │
               │ mTLS (node cert signed by user's CA)
               │
               ▼
┌─────────────────────────────────────────────────────────────────────────────┐
│                           NODES (nitellad)                                  │
...
│  │  • Cert (signed by CA)  │     │  • Cert (signed by CA)  │               │
│  │  • CLI CA cert          │     │  • CLI CA cert          │               │
│  │                         │     │                         │               │
│  │  Decrypts E2E commands  │     │  Decrypts E2E commands  │               │
│  │  Encrypts E2E responses │     │  Encrypts E2E responses │               │
│  └─────────────────────────┘     └─────────────────────────┘               │
│                                                                             │
└─────────────────────────────────────────────────────────────────────────────┘
```
...
1. Hub first launch → generates self-signed CA → displays fingerprint
2. Hub operator shares fingerprint with users out-of-band
3. User first connect → CLI detects self-signed → shows TOFU prompt
4. User verifies fingerprint and accepts
5. Certificate saved locally → future connections verify against saved cert

```
╔══════════════════════════════════════════════════════════════╗
║         ⚠️  FIRST CONNECTION - VERIFY HUB IDENTITY ⚠️          ║
╠══════════════════════════════════════════════════════════════╣
║  Hub:        hub.example.com:50052                           ║
║  Subject:    Nitella Hub                                     ║
║  Emoji:      🔥 🌟 🎸 🚀                                      ║
...
║  Verify fingerprint matches Hub operator's published value!  ║
╚══════════════════════════════════════════════════════════════╝
...
Root CA (Mobile/BIP-39 derived)
    └── Node Certificate (issued during registration)
```
...
└────┬────┘                           └────┬────┘
     │                                     │
     │  1. AuthChallenge + CertPEM         │
     │────────────────────────────────────>│
     │                                     │
     │     Verify cert signed by CA        │
     │                                     │
     │  2. AuthResponse + CertPEM + Sig    │
     │<────────────────────────────────────│
     │                                     │
     │  Verify cert signed by CA           │
...
```
┌─────────────────────────────────────────────────────────────────────┐
│                      ENCRYPTED_LOGS TABLE                            │
├─────────────────────────────────────────────────────────────────────┤
│                                                                     │
│  ┌────────────┬─────────────────┬──────────────────┬──────────┐    │
│  │ node_id    │ routing_token   │ encrypted_blob   │ timestamp│    │
│  ├────────────┼─────────────────┼──────────────────┼──────────┤    │
│  │ node-a     │ a7f3b2c1d4...   │ <encrypted>      │ 2024-... │    │
│  │ node-b     │ x9y8z7w6v5...   │ <encrypted>      │ 2024-... │    │
│  └────────────┴─────────────────┴──────────────────┴──────────┘    │
│                                                                     │
│  Hub CAN see:     ✓ node_id, routing_token, timestamp, blob size   │
│  Hub CANNOT see:  ✗ log content, log level, log message            │
│                                                                     │
└─────────────────────────────────────────────────────────────────────┘
```
